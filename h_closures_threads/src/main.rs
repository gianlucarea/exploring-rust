...
fn main() {
    ...
    // Wrap the receiver in an Arc and Mutex for shared ownership
    let rx = Arc::new(Mutex::new(rx));

    // Spawn the first child thread
    let rx1 = Arc::clone(&rx);
    let handle1 = thread::spawn(move || {
        while let Ok(value) = rx1.lock().unwrap().recv() {
